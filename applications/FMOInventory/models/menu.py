...
response.subtitle = T('FMO Inventory!')

#http://dev.w3.org/html5/markup/meta.name.html
response.meta.author = 'Henry Velez'
...
response.menu+= [
    (T('Administrador'), False, URL('default','list_laptop'), [])
    ]
response.menu+= [
    (T('Documents'), False, URL('default','list_docs'), [])
    ]
#response.menu+=[
#    (T('Workshop'), False, URL('default', 'list_parts'), [])
#    ]
response.menu+=[
    (T('Tools'), False, URL('default', 'view_tools'), [])
    ]
#response.menu+=[
#    (T('Supplies'), False, URL('default', 'list_insumos'), [])
 #   ]   
response.menu+=[
    (T('Movements'), False, URL('default', 'list_movs'), [])
    ]       
